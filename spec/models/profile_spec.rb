...
RSpec.describe Profile, type: :model do
  describe 'バリデーション' do
    it 'heightが0より大きい整数であること', skip: '一時的にskip' do
    ...
    it 'target_weightが0より大きい整数であること', skip: '一時的にskip' do
  ...
  describe 'enum/デフォルト値' do
    it 'genderの初期値はmanであること', skip: '一時的にskip' do
      profile = Profile.new
      expect(profile.gender).to eq('man')
    end
    it 'training_intensityの初期値はlowであること', skip: '一時的にskip' do
      profile = Profile.new
      expect(profile.training_intensity).to eq('low')
    end
    it 'genderの値が正しいこと', skip: '一時的にskip' do
      expect(Profile.genders.keys).to contain_exactly('man', 'woman', 'other')
    end
    it 'training_intensityの値が正しいこと', skip: '一時的にskip' do
  ...
  describe '独自メソッド' do
    describe '#condition_key' do
      it 'genderとtraining_intensityが両方nilならnilを返す', skip: '一時的にskip' do
        profile = Profile.new(gender: nil, training_intensity: nil)
        expect(profile.condition_key).to be_nil
      end
      it 'genderとtraining_intensityが両方存在すればkeyを返す', skip: '一時的にskip' do
