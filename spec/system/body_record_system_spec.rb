...
RSpec.describe '身体情報入力・編集', type: :system do
  ...
  it '新規で身体情報を入力・登録できる', skip: '一時的にskip' do
    # 今日の日付を選択し、入力ボタンを押す
  ...
  it '既存の身体情報を編集できる', skip: '一時的にskip' do
    # 事前にレコード作成
  ...
  it '体重・体脂肪率が未入力でも登録できる', skip: '一時的にskip' do
    within('.simple-calendar') do
      all('a', text: Date.current.day.to_s).first.click
    end
    click_link '＋ 身体情報を入力・編集'
  ...
  it '体重が300を超えるとバリデーションエラーになる', skip: '一時的にskip' do
    within('.simple-calendar') do
      all('a', text: Date.current.day.to_s).first.click
    end
    click_link '＋ 身体情報を入力・編集'
  ...
  it '体脂肪率が100を超えるとバリデーションエラーになる', skip: '一時的にskip' do
    within('.simple-calendar') do
      all('a', text: Date.current.day.to_s).first.click
    end
    click_link '＋ 身体情報を入力・編集'
  ...
  it '体重・体脂肪率に負の値を入力した場合はバリデーションエラーになる', skip: '一時的にskip' do
    within('.simple-calendar') do
      all('a', text: Date.current.day.to_s).first.click
    end
    click_link '＋ 身体情報を入力・編集'
